...
try:
    ...
    # 메인 화면에 2개의 열(Column)을 생성하여 선택 상자를 가로로 배치합니다.
    ...
    with col1:
        ...
        selected_gender = st.selectbox("👤 성별을 선택하세요", gender_list, index=default_gender_idx)

    # 선택된 성별의 데이터만 먼저 필터링하여 연령대 목록을 실시간으로 연동
    ...
    with col2:
        # 메인보드 설정 2: 연령대 선택
        selected_age = st.selectbox("🎂 연령대를 선택하세요", age_list, index=default_age_idx)
